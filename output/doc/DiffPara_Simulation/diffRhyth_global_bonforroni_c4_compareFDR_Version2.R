...
method.dir = list(m1 = list(name = "1Bonferroni2BH3Individual", 
                  ...
                  m2 = list(name = "1BH2Bonferroni", 
                            dir = "res_circacompare_1BH2Bonferroni0/Version2_"), 
                  m3 = list(name = "BH", 
                            dir = "res_circacompare_BH0/Version2_"), 
...
p3 = ggplot(data = res.tab.F, aes(x = test, y = F, fill = method))+
  geom_boxplot()+ scale_y_continuous(breaks=seq(0,1,.05))
library(gridExtra)
pdf(paste0(dir, "/FDR control compare Version2.pdf"), width = 20)
...
p3 = ggplot(data = sum.subcat.tab, aes(x = type, y = F.score, fill = method))+
  geom_boxplot()+ scale_y_continuous(breaks=seq(0,1,.05))
library(gridExtra)
pdf(paste0(dir, "/FDR control compare_subcategories_Version2.pdf"), width = 25)
grid.arrange(p1, p2, p3, ncol = 3)
dev.off()
...
p = ggplot(data = FDR.component.tab, aes(x = type, y = measure, fill = method))+
  geom_boxplot()+
  facet_grid(.~FDR.type)
pdf("FDRcomponent_Version2.pdf", width = 25, height = 10)
print(p)
dev.off()


#check if there is global difference then there must be individual difference
check1= mclapply(1:100, function(r){
  
  one.res = do.call(rbind.data.frame, lapply(method.dir, function(m){
    one.method = readRDS(paste0(dir, "/", m$dir, "res", "_n", r, ".rds"))
    global.individual.match = apply(one.method, 1, function(a){
      one.match = one.method$ind.global==(one.method$ind.Mesor|one.method$ind.phase|one.method$ind.amp)
      return(one.match)
    })
    return(all(global.individual.match))
  }))
  
}, mc.cores = 20)

all(do.call(c, lapply(check1, function(a){a[1, 1]})))
all(do.call(c, lapply(check1, function(a){a[2, 1]})))
all(do.call(c, lapply(check1, function(a){a[3, 1]})))
all(do.call(c, lapply(check1, function(a){a[4, 1]})))
all(do.call(c, lapply(check1, function(a){a[5, 1]})))
all(do.call(c, lapply(check1, function(a){a[6, 1]})))
#only the first method does not match, the question is: which one: global changed but no individual, or there is individual but not global?
r = 1
one.method = readRDS(paste0(dir, "/", method.dir[[1]]$dir, "res", "_n", r, ".rds"))
one.method[!one.method$ind.global==(one.method$ind.Mesor|one.method$ind.phase|one.method$ind.amp), ]
the.BH.p.cutoff = max(one.method$p.InGeneCorrected[one.method$p.BH<0.05])

#I found that this is actually no problem, just use <= the.BH.p.cutoff instead of < the.BH.p.cutoff 
