...
method.dir = list(m1 = list(name = "1Bonferroni2BH3Individual", 
                            dir = "/res_cosinor_permutation/res_permutation_1Bonferroni2BH3Individual"), 
                  m2 = list(name = "1BH2Bonferroni", 
                            dir = "/res_cosinor_permutation/res_permutation_1BH2Bonferroni0"), 
                  m3 = list(name = "BH", 
                            dir = "/res_cosinor_permutation/res_permutation_BH0"), 
                  m4 = list(name = "AWFisher",
                            dir = "/res_cosinor_permutation/res_permutation_AWFisher"), 
                  m2b = list(name = "1BH2Bonferroni_divide3", 
                             dir = "/res_cosinor_permutation/res_permutation_1BH2Bonferroni"), 
                  m3b = list(name = "BH_divide3", 
                             dir = "/res_cosinor_permutation/res_permutation_BH"))
all.res = mclapply(1:100, function(r){

  one.res = do.call(rbind.data.frame, mclapply(method.dir, function(m){
    one.method = readRDS(paste0(dir, m$dir, "/res", "_n", r, ".rds"))
    one.row = data.frame(r = r,
               method = m$name, 
...
p3 = ggplot(data = res.tab.F, aes(x = test, y = F, fill = method))+
  geom_boxplot()+ scale_y_continuous(breaks=seq(0,1,.05))
library(gridExtra)
pdf(paste0(dir, "/FDR control compare in permutation.pdf"), width = 20)
grid.arrange(p1, p2, p3, ncol = 3)
dev.off()


sum(!(is.na(res.tab$FDR.offset)))
sum(is.na(res.tab$FDR.offset))
...
p3 = ggplot(data = sum.subcat.tab, aes(x = type, y = F.score, fill = method))+
  geom_boxplot()+ scale_y_continuous(breaks=seq(0,1,.05))
library(gridExtra)
pdf(paste0(dir, "/FDR control compare_subcategories_permutation.pdf"), width = 25)
grid.arrange(p1, p2, p3, ncol = 3)
dev.off()
...
p = ggplot(data = FDR.component.tab, aes(x = type, y = measure, fill = method))+
  geom_boxplot()+
  facet_grid(.~FDR.type)
pdf("FDRcomponent_permutation.pdf", width = 25, height = 10)
print(p)
dev.off()
dev.off()