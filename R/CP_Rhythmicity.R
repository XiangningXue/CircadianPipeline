...
#' }
#' @param x2 group II data. Same as x1 for formats.
...
#' @param period integer. The length of the oscillation cycle. Default is 24 for circadian signals.
#' @param alpha integer. Threshold for rhythmicity p-value in joint rhythmicity categorization. If CI = TRUE, (1-alpha) confidence interval for parameters will be returned.
#' @param alpha.FDR integer. Threshold for rhythmicity p-value in joint rhythmicity categorization adjusted for global FDR control.
...
#'
#' @return A list of origical x input and rhythmicity analysis estimates. If given two data sets, joint rhythmicity categorization will also be available in component rhythm.joint
#' @export
#'
#' @examples
...
CP_Rhythmicity = function(x1, x2=NULL, method = "Sidak_FS", period = 24, alpha = 0.05, alpha.FDR = 0.05, CI = FALSE, p.adjust.method = "BH", parallel.ncores  = 1){
...
#'
#' @examples
#' #Re-calculate TOJR for CP_rhythmicity(x1, x2) output with q-value cutoff 0.1
#' x = CP_sim_data(ngene=1000, nsample=30, A1=c(1, 3), A2=c(1, 3),
#' phase1=c(0, pi/4), phase2=c(pi/4, pi/2),
#' M1=c(4, 6), M2=c(4, 6), sigma1=1, sigma2=1)
#' rhythm.res = CP_Rhythmicity(x1 = x[[1]], x2 = x[[2]])
...
#'
#' #Calculate TOJR for two CP_rhythmicity(x1, x2 = NULL) outputs with p-value cutoff 0.05
#' x = CP_sim_data(ngene=1000, nsample=30, A1=c(1, 3), A2=c(1, 3),
#' phase1=c(0, pi/4), phase2=c(pi/4, pi/2),
#' M1=c(4, 6), M2=c(4, 6), sigma1=1, sigma2=1)
