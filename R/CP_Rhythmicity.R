...
#'
#' @param x  A list containing the following component: data: dataframe or matrix with rows of genes and columns of samples; tod: time of death or time of expression corresponding to the columns in data component; label: the gene names or other labels of the genes
...
#' x = list(data = noise.mat + signal.mat, tod = x.time, label = paste("gene", seq_len(100)))
...
CP_Rhythmicity = function(x = list(data = data1, tod = tod, label = label),
  ...
  if(method == "OLS"){
    rhythmicity.list = option_parallel(1:nrow(X), function(a){
      ...
      res.onerow = as.data.frame(list(M = res$M$est, M.ll = res$M$CI_M[1], M.ul = res$M$CI_M[2],
                                      A = res$A$est, A.ll = res$A$CI_A[1], A.ul = res$A$CI_A[2],
                                      phase = res$phase$est, phase.ll = res$phase$CI_phase[1], phase.ul = res$phase$CI_phase[2], peak = res$peak,
                                      Fstat = res$test$Fstat, sigma2 = res$test$sigma2, R2 = res$test$R2, pvalue = res$test$pval))
    }, parallel, cores)
